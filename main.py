...
def get_data(event=None):
   ...
   loadingLabel=Label(root, text="Loading... ", font=("Arial",13, "italic"), bg="#c3e090")
   ...
   if zip.isdigit() == False or len(zip) != 5:
    ...
    loadingLabel.update()
    tkinter.messagebox.showinfo("ERROR", "Please enter valid U.S. ZIP code.")
    #print("invalid ZIP code")
    return
   
   geoAPI_url = "http://api.openweathermap.org/geo/1.0/zip?zip=" + zip + ",US&appid=API_KEY_HERE" #<---Enter API key
   ...
   weatherAPI_url = "https://api.openweathermap.org/data/3.0/onecall?lat=" + lat + "&lon=" + lon + "&exclude=minutely,hourly,daily&units=imperial&appid=" #<---Enter API key
   weatherData = requests.get(weatherAPI_url).json()
   #print(weatherData) 
   loadingLabel.config(text="Done!  ")
   weatherDataLabel.config(text=weatherData["current"]["weather"][0]["description"])
   windDataLabel.config(text=str(weatherData["current"]["wind_speed"]) + "mph")
   ...
   summaryAPI_url = "https://api.openweathermap.org/data/3.0/onecall/overview?lat=" + lat + "&lon=" + lon + "&units=imperial&appid=API_KEY_HERE" #<---Enter API key
   summaryData = requests.get(summaryAPI_url).json()
   sumText = textwrap.fill(summaryData["weather_overview"], width = 40)
#    print(sumText)
   weatherSumLabel=Label(root,text="Weather Overview Description",font=("Trebuchet MS", 20, "bold"),bg="#c3e090")
...
dropDown=ttk.Combobox(root,values=time_intervals,textvariable=timeInt,font=('Arial',7), state="readonly")
dropDown.place(x=350,y=105,height=30, width=50)

infoLabel=Label(root,text="Enter U.S. ZIP code",font=("Trebuchet MS", 20, "bold"),bg="#c3e090")
infoLabel.place(x=90,y=20)
weatherLabel=Label(root,text="Weather",font=("Trebuchet MS",15),bg="#c3e090")
weatherLabel.place(x=80,y=160)
windLabel=Label(root,text="Wind",font=("Trebuchet MS",15),bg="#c3e090")
windLabel.place(x=80,y=240)
tempLabel=Label(root,text="Temperature",font=("Trebuchet MS",15),bg="#c3e090")
tempLabel.place(x=80,y=200)
humidLabel=Label(root,text="Humidity",font=("Trebuchet MS",15),bg="#c3e090")
humidLabel.place(x=80,y=280)
hPaLabel=Label(root,text="Pressure",font=("Trebuchet MS",15),bg="#c3e090")
hPaLabel.place(x=80,y=320)

weatherDataLabel=Label(root,text=" ",font=("Trebuchet MS",15),bg="#c3e090",fg="#008B00")
weatherDataLabel.place(x=265,y=160)
windDataLabel=Label(root,text=" ",font=("Trebuchet MS",15),bg="#c3e090",fg="#008B00")
windDataLabel.place(x=265,y=240) 
tempDataLabel=Label(root,text=" ",font=("Trebuchet MS",15),bg="#c3e090",fg="#008B00")
tempDataLabel.place(x=265,y=200)
humidDataLabel=Label(root,text=" ",font=("Trebuchet MS",15),bg="#c3e090",fg="#008B00")
humidDataLabel.place(x=265,y=280)
hPaDataLabel=Label(root,text=" ",font=("Trebuchet MS",15),bg="#c3e090",fg="#008B00")
hPaDataLabel.place(x=265,y=320)


btn=Button(root,text="Check Weather",font=("Arial",13),bg="#E6E6FA",bd=5,command=get_data, width=15)
